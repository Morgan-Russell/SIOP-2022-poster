...
As a whole, the engagement survey displayed high internal consistency ($\alpha$ = `r psych::alpha(CFAdata)$total$raw_alpha %>% round(2)`). Coefficient alphas for subscales ranged from `r psych::alpha(absorption)$total$raw_alpha %>% round(2)` for absorption to `r psych::alpha(affective)$total$raw_alpha %>% round(2)` for affective. The bifactor model displayed strong fit, $\chi^2$(`r fitindices$FIT[4]`, N = `r nrow(CFAdata)`) = `r fitindices$FIT[3] %>% round(3) %>% formatC(digits = 2, format = 'f')`, *p* \< .001, CFI = `r fitindices$FIT[9] %>% round(3) %>% formatC(digits = 2, format = 'f')`, AIC = `r fitindices$FIT["aic"]  %>% round(3) %>% formatC(digits = 2, format = 'f')`, RMSEA = `r fitindices$FIT["rmsea"] %>% round(3) %>% formatC(digits = 2, format = 'f')`.
...
```{r, corrplot, fig.cap="Correlations among subscales", out.width="80%", fig.align='center'}
svg("corrplot.svg")
